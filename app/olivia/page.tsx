...
export default function OliviaPage() {
  ...
  // Set NEXT_PUBLIC_VIDEO_URL_ in Vercel environment variables with your video URL
  ...
  useEffect(() => {
    setIsMounted(true);
    const videoUrl = process.env.NEXT_PUBLIC_VIDEO_URL_ || '';
    ...
    if (!videoUrl) {
      console.warn('[Olivia Video] NEXT_PUBLIC_VIDEO_URL_ is not set. Please configure it in Vercel environment variables.');
    }
  ...
  useEffect(() => {
    ...
    return () => {
      setYoutubePlayer((currentPlayer: YTPlayer | null) => {
        if (currentPlayer) {
          try {
  ...
  useEffect(() => {
    if (!videoSrc) {
      console.log('No video source provided. Set NEXT_PUBLIC_VIDEO_URL_ in Vercel environment variables.');
      return;
    }
            ...
            {videoSrc ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/80 rounded-3xl" style={{ borderRadius: '24px' }}>
                <div className="text-center text-white p-6 max-w-md">
                  <p className="text-2xl mb-4 font-semibold">Olivia Video</p>
                  <p className="text-lg mb-2">Video will be available shortly</p>
                  <p className="text-sm text-gray-300">
                    Configure NEXT_PUBLIC_VIDEO_URL_ in your environment variables with a YouTube or video URL.
                  </p>
                </div>
              </div>
            )}
            ...
            {videoError && !isYouTube && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/80 rounded-3xl" style={{ borderRadius: '24px' }}>
                <div className="text-center text-white p-6 max-w-md">
                  ...
                  <p className="text-lg mb-2">Video will be available shortly</p>
                  <p className="text-sm text-gray-300">
                    Our video is being finalized and will appear here soon. Please check back later.
                  </p>
                </div>
              </div>
            )}
